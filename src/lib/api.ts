...
export async function quiz(slug: String) {
	try {
		let res = await client.get(`/quizzes/${slug}`);
		return res.data;
	} catch (err) {
		throw err;
	}
...
export async function commentSend(id: string, comment: string, user_id: string) {
	try {
...
export async function postSend(content: string, user_id: string, title: string) {
	try {
